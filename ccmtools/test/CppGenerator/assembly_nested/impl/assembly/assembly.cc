...
void 
Assembly::build(Components::CCMObject::SmartPtr facadeComponent)
  throw (Components::CreateFailure)
{
  int error = 0;
  ...
  try 
  {
    ...
    BasicTestHome::SmartPtr basicTestHome(dynamic_cast<BasicTestHome*>(
       homeFinder->find_home_by_name("BasicTestHome").ptr()));
    UserTestHome::SmartPtr userTestHome(dynamic_cast<UserTestHome*>(
       homeFinder->find_home_by_name("UserTestHome").ptr()));

    // create components
    superTest = SuperTest::SmartPtr(dynamic_cast<SuperTest*>(facadeComponent.ptr()));
