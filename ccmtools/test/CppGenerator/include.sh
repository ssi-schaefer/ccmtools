...
idldir=${abssrcdir}/test/CppGenerator/include
...
test -z "${ret}" && ${top_srcdir}/test/CppGenerator/test-loader.sh \
  "_include_base" "-I${idldir}" "${idldir}/Lcd.idl ${idldir}/Console.idl" \
  "keep-install" || ret=1

test -z "${ret}" && ${top_srcdir}/test/CppGenerator/test-loader.sh \
  "include" "-I${idldir}" "${idldir}/Hello.idl" "keep-install" || ret=1

test -z "${ret}" && ret=0

cd ${sandbox_dir}
ccmtools-c++-uninstall -p _include_base
ccmtools-c++-uninstall -p include
