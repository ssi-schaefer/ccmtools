/**
 * CCM Tools : C++ Code Generator Library Leif Johnson <leif@ambient.2y.net> Egon Teiniker
 * <egon.teiniker@salomon.at> Copyright (C) 2002 - 2005 Salomon Automation
 * 
...
abstract public class CppGenerator extends CodeGenerator
{
    ...
    private final static String[] _reserved = { "and", "and_eq", "asm", "auto", "bitand", "bitor",
            ...
            "inline", "int", "long", "mutable", "namespace", "new", "not", "not_eq", "operator",
    ...
    private final static String[] _language = { "", "::wamas::platform::utils::Value", // PK_ANY
            "bool", // PK_BOOLEAN
            "char", // PK_CHAR
            "double", // PK_DOUBLE
    ...
    public CppGenerator( String sublang, UserInterfaceDriver d, File out_dir, String[] output_types )
            throws IOException
    {
    ...
    protected String getLocalCxxGenIncludeName( MContained node, String separator )
    {
        ...
        {
            out.append(Text.joinList(separator, getLocalCxxGenNamespaceList(node))).append(
                separator);
        }
    ...
    /**
     * Overide the CodeGenerator method to return the base types of a MInterface model element.
     */
    ...
    /**
     * Override the CodeGenerator method to handle local C++ namespace artifacts.
     */
    ...
    /**
     * Overide the CodeGenerator method to use the local C++ namespaces and identifier.
     */
    ...
    /**
     * Overide the CodeGenerator method to use the local C++ namespace.
     */
    ...
    /**
     * Implement the abstract method from CodeGenerator and uses local C++ namesapce.
     */
    ...
    /**
     * Implement the abstract CodeGenerator method to define local C++ types (depending on a given
     * model element type).
     */
    public String getLanguageType( MTyped object )
    {
        ...
        if (object instanceof MParameterDef)
        {
            ...
            { // in
                ...
                if ((idl_type instanceof MTypedefDef)
                        ...
                        || (idl_type instanceof MStringDef) || (idl_type instanceof MWstringDef)
                        || (idl_type instanceof MFixedDef) || (idl_type instanceof MInterfaceDef))
                {
                    suffix = "&";
                }
            }
        ...
        {
            /*
             * This code defines the IDL -> C++ mapping of arrays: long x[7] -> std::vector <long>
    ...
    // ====================================================================
    /**
     * Acknowledge and process a closing node during graph traversal. If the node is an MContainer
     * type, pop the namespace (this will remove our base namespace that we pushed, in theory (tm)).
     * If the node is of the correct type and defined in the original parsed file, write code for
     * this node.
     * 
     * @param node the node that the graph traverser object just finished investigating.
     * @param scope_id the full scope identifier of the node. This identifier is a string containing
     *            the names of ancestor nodes, joined together with double colons.
     */
     ...
     * 
     * This function performs some common value parsing in the CCM MOF library. More specific value
     * parsing needs to be provided in the subclass for a given language, in the subclass'
     * getLocalValue function. Subclasses should call this function first and then perform any
     * subclass specific value manipulation with the returned value.
     * 
     * @param variable The variable name to get a value for.
     * @return the value of the variable available from the current output_variables hash table.
    ...
    /**
     * Load an appropriate template (based on the value in the template_name argument) for the given
     * child, and fill out its variable information.
     * 
     * @param child MInterfaceDef node to gather information from.
     * @param template_name the name of the template to load for variable substitution.
     * @param attribute true if we should fill in attribute information. If false, then we will fill
     *            in operation information.
     * @return a string containing the variable-substituted template requested.
     */
    protected String fillTwoStepTemplates( MInterfaceDef child, String template_name,
            boolean attribute )
    {
        ...
        while (!ifaces.empty())
        {
            ...
            List contents = iface.getFilteredContents((attribute) ? MDefinitionKind.DK_ATTRIBUTE
    ...
    /**
     * Get a variable hash table sutable for filling in the template from the fillTwoStepTemplates
     * function. This function handles attributes.
     * 
     * @param iface the interface from which we're starting the two step operation.
     * @param attr the particular interface attribute that we're filling in a template for.
     * @param contained the container in which the given interface is defined.
     * @return a map containing the keys and values needed to fill in the template for this
     *         interface.
     */
    protected Map getTwoStepAttributeVariables( MAttributeDef attr, MContained contained )
    {
    ...
    /**
     * Get a variable hash table sutable for filling in the template from the fillTwoStepTemplates
     * function. This version of the function handles operations from the given interface.
     * 
     * @param operation the particular interface operation that we're filling in a template for.
     * @param container the container in which the given interface is defined.
     * @return a map containing the keys and values needed to fill in the template for this
     *         interface.
     */
    abstract protected Map getTwoStepOperationVariables( MOperationDef op, MContained contained );
    ...
    protected String data_MFactoryDef( String dataType, String dataValue )
    {
        ...
        {
            dataValue = getLocalCxxGenNamespace(component, Text.SCOPE_SEPARATOR) + "CCM_"
                    + component.getIdentifier();
        }
        if (dataType.equals("CCMComponentType"))
        {
            dataValue = getLocalCxxNamespace(component, Text.SCOPE_SEPARATOR) + "CCM_"
                    + component.getIdentifier();
        }
        ...
        {
            dataValue = getLocalCxxGenNamespace(home, Text.SCOPE_SEPARATOR) + "CCM_"
                    + home.getIdentifier();
        }
        else
        {
    ...
    protected String data_MOperationDef( String data_type, String data_value )
    {
        ...
        else if (data_type.startsWith("MParameterDef") && data_value.endsWith(", "))
        {
    ...
    protected String data_MProvidesDef( String data_type, String data_value )
    {
        ...
        else if (data_type.startsWith("OpenNamespace") || (data_type.startsWith("CloseNamespace")))
        {
    ...
    protected String data_MSupportsDef( String data_type, String data_value )
    {
        ...
        {
            return getLocalCxxNamespace(iface, Text.SCOPE_SEPARATOR) + "CCM_"
                    + iface.getIdentifier();
        }
        else if (data_type.startsWith("MOperation"))
        {
    ...
    /**
     * Get type and name information about the parameters for the given operation. This will return
     * a comma-separated string, i.e. <type1> <name1>, <type2><name2>, ... ,<typeN><nameN>for
     * this operation's parameters.
     * 
     * @param op the operation to investigate.
     * @return a comma separated string of the parameter information requested for this operation.
     */
    ...
    /**
     * Get name information about the parameters for the given operation. This will return a
     * comma-separated string, i.e. <name1>, <name2>, ... , <nameN>for this operation's parameters.
     * 
     * @param op the operation to investigate.
     * @return a comma separated string of the parameter information requested for this operation.
     */
     ...
     * 
     * @param op the operation to investigate.
     * @return a string containing C++ code describing the exceptions that this operation throws. If
     *         there are no exceptions, this returns an empty string. If there are exceptions, it
     *         returns something like "throw ( exception, exception, ... )".
     */
