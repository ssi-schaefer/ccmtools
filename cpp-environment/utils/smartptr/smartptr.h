...
public:
   ...
   /** @name see the big note above for copy semantics */
   //@{
   ...
   /** @name simple wrappers to accommodate null pointers */
   //@{
   ...
   //@}
    ...
    typedef std::map<std::size_t, long> NewStatistics;
    static NewStatistics new_statistics_;
#endif

private:
    atomic_count refcount_;

public:
   LTA_MEMDECL(20061107);
};
LTA_STATDEF(RefCounted, 20061107);
...
public:
   ...
   /** Copy constructor, template version (to be able to assign
       ...
       by one. */
   template<class RHS>
   SmartPtr(const SmartPtr<RHS>& p): ptr_(0) { operator=(p); }
